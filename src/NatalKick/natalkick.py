...
def parser():
    parser = argparse.ArgumentParser(description="Script to estimate Natal kick for a system based on location,\
     proper motion, radial velocity and distance with their uncertainties.\
     ")
...
def main():
    ...
    print('System natal kick (km/s):', np.percentile(kicks, 50), '(-',
          np.percentile(kicks, 50) - np.percentile(kicks, 16), '/+',
          np.percentile(kicks, 84) - np.percentile(kicks, 50), ')')
